#if !defined(BASE_HTTP_URI_DECODE)
#define BASE_HTTP_URI_DECODE

#include <string>

namespace base {
namespace http {
namespace detail {

template <typename CharT>
CharT letter_to_hex(CharT in) {
  switch (in) {
    case '0':
    case '1':
    case '2':
    case '3':
    case '4':
    case '5':
    case '6':
    case '7':
    case '8':
    case '9':
      return in - '0';
    case 'a':
    case 'b':
    case 'c':
    case 'd':
    case 'e':
    case 'f':
      return in + 10 - 'a';
    case 'A':
    case 'B':
    case 'C':
    case 'D':
    case 'E':
    case 'F':
      return in + 10 - 'A';
    default:
      return 0;
  }
  return CharT();
}

}  // namespace detail

template <class InputIterator, class OutputIterator>
OutputIterator urlDecode(const InputIterator &in_begin,
                         const InputIterator &in_end,
                         const OutputIterator &out_begin) {
  typedef typename std::iterator_traits<InputIterator>::value_type value_type;

  InputIterator it = in_begin;
  OutputIterator out = out_begin;
  while (it != in_end) {
    if (*it == '%') {
      if (++it == in_end) {
        // throw std::out_of_range("unexpected end of stream");
        return out;
      }
      value_type v0 = detail::letter_to_hex(*it);
      if (++it == in_end) {
        // throw std::out_of_range("unexpected end of stream");
        return out;
      }
      value_type v1 = detail::letter_to_hex(*it);
      ++it;
      *out++ = 0x10 * v0 + v1;
    } else if (*it == '+') {
      *out++ = ' ';
      ++it;
    } else {
      *out++ = *it++;
    }
  }
  return out;
}

template <class SinglePassRange, class OutputIterator>
inline OutputIterator urlDecode(const SinglePassRange &range,
                                const OutputIterator &out) {
  return urlDecode(std::begin(range), std::end(range), out);
}

inline std::string urlDecode(const std::string &input) {
  std::string decoded;
  urlDecode(input, std::back_inserter(decoded));
  return decoded;
}

}  // namespace http
}  // namespace base

#endif  // BASE_HTTP_URI_DECODE
